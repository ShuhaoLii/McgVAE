...
from McgVAE.McgVAE import McgVAE
...
def get_x_y_in_correct_dims(x, y,type):
    ...
    if type == 'lane':
        ...
        y = y[..., :args.output_dim].view (args.horizon, batch_size,
...
def evaluate(model, dataset='val', batches_seen=0):
    ...
    with torch.no_grad ():
        ...
        for _, ((x, y), (x_road, y_road)) in enumerate (zip (val_iterator, val_iterator_road)):
            ...
            x_road = x_road.unsqueeze (4).permute (1, 0, 3, 2, 4)
            y = y.squeeze (3).permute (1, 2, 0)
            y_road = y_road.squeeze (3).permute (1, 2, 0)
            if args.isNoise:
                x = add_noise(x,args.Noise_ratio)
            if args.isMissing:
                x = set_na_randomly(x,args.Missing_ratio)
            output_road,output = model (x_road,x)
            loss = compute_loss (y, output)
            losses.append (loss.item ())
            y_truths.append (y.cpu ())
            y_preds.append (output.cpu ())
            y_truths_road.append (y_road.cpu ())
            y_preds_road.append (output_road.cpu ())

        mean_loss = np.mean (losses)
        y_preds = np.concatenate (y_preds, axis=1)
        y_truths = np.concatenate (y_truths, axis=1)  # concatenate on batch dimension
        y_preds_road = np.concatenate (y_preds_road, axis=1)
        y_truths_road = np.concatenate (y_truths_road, axis=1)  # concatenate on batch dimension

        y_truths_scaled = []
        y_preds_scaled = []
        y_truths_scaled_road = []
        y_preds_scaled_road = []
        ...
        for t in range (y_preds_road.shape[0]):
            y_truth_road = standard_scaler_road.inverse_transform (y_truths_road[t])
            y_pred_road = standard_scaler_road.inverse_transform (y_preds_road[t])
            y_truths_scaled_road.append (y_truth_road)
            y_preds_scaled_road.append (y_pred_road)

        y_preds_scaled = np.array (y_preds_scaled)
        y_truths_scaled = np.array (y_truths_scaled)
        y_preds_scaled_road = np.array (y_preds_scaled_road)
        y_truths_scaled_road = np.array (y_truths_scaled_road)
        mae, mape, rmse = calc_metrics (y_preds_scaled, y_truths_scaled, null_val=0.0)
        mae_road, mape_road, rmse_road = calc_metrics (y_preds_scaled_road, y_truths_scaled_road, null_val=0.0)
        print ('lane  mae:', mae.item (), 'mape:', mape.item (), 'rmse:', rmse.item ())
        print ('road  mae:', mae_road.item (), 'mape:', mape_road.item (), 'rmse:', rmse_road.item ())
...
def add_noise(tensor, k, noise_type='uniform', mean=15, std=10, low=-15, high=15):
    """
    在PyTorch张量的seq_len维度上添加噪声。

    :param tensor: 输入的PyTorch张量，形状为(batch_size, seq_len, input_dim, num_node, out_dim)
    :param k: 噪声比例，决定了要对多少比例的seq_len维度上的元素添加噪声
    :param noise_type: 噪声类型，支持'gaussian'高斯噪声和'uniform'均匀噪声
    :param mean: 高斯噪声的平均值
    :param std: 高斯噪声的标准差
    :param low: 均匀噪声的最低值
    :param high: 均匀噪声的最高值
    ...
    if noise_type == 'gaussian':
        ...
        noise = (high - low) * torch.rand (tensor.shape) + low
        noise = noise.cuda(args.gpu)
    else:
    ...
    for i in range (tensor.shape[0]):  # batch_size维度
        for j in range (tensor.shape[2]):  # input_dim维度
            for m in range (tensor.shape[3]):  # num_node维度
                for n in range (tensor.shape[4]):  # out_dim维度
                    indices = torch.randperm (seq_len)[:num_to_select].cuda(args.gpu)
                    tensor[i, indices, j, m, n] += noise[i, indices, j, m, n]
...
if __name__ == '__main__':
    ...
    data_lane = load_dataset (args.dataset_dir, args.batch_size, args.horizon)
    if args.dataset_name == "PEMS":
        data_road = load_dataset('./Datasets/PeMS_Road_43', args.batch_size, args.horizon )
    else:
        data_road = load_dataset ('./Datasets/HuaNan_Road_72', args.batch_size, args.horizon)
    standard_scaler_lane = data_lane['scaler']
    standard_scaler_road = data_road['scaler']

    model =McgVAE(args.seq_len, args.road_num_nodes, args.lane_num_nodes, args.input_dim, args.output_dim ,args.horizon)
    model.to (device)
    ...
    for epoch_num in range (0, args.epochs):
        ...
        for _, ((x, y),(x_road,y_road)) in enumerate (zip(train_iterator,train_iterator_road)):
            ...
            if args.isMissing:
                x = set_na_randomly(x,args.Missing_ratio)
            output_road,output = model (x_road,x)
            loss1 = compute_loss (y, output)
            loss2 = compute_loss (y_road, output_road)
            loss = loss1+loss2
